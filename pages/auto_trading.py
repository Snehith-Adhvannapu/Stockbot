...
def show():
    ...
    <div style='text-align: center; padding: 1.5rem 0;'>
        ...
        <p style='color: #666; font-size: 1.1rem;'>Let AI trade for you automatically - It's easy!</p>
    </div>
    ...
    if bot:
        ...
        with col1:
            if bot.running:
                st.success("✅ Your bot is RUNNING - It's watching the market for you!")
            else:
                st.warning("⏸️ Your bot is SET UP but STOPPED - Click 'Start Trading' below")
    ...
    if not bot:
        st.markdown("## Step 1️⃣: Pick Your Stocks")
        st.markdown("Choose 3-10 stocks you want the bot to watch and trade automatically.")
        
        watchlist_input = st.text_area(
            "✏️ Type stock symbols here (one per line):",
            value="AAPL\nMSFT\nGOOGL\nTSLA\nAMZN",
            height=140,
            help="Popular stocks: AAPL (Apple), MSFT (Microsoft), TSFT (Tesla)"
        )
        ...
        if len(watchlist) >= 3:
            ...
            st.warning(f"⚠️ Add at least 3 stocks (you have {len(watchlist)})")
        
        st.markdown("---")
        st.markdown("## Step 2️⃣: Choose Trading Style")
        ...
        with col1:
            ...
            strategy_map = {
                "🎯 Hybrid (RECOMMENDED FOR BEGINNERS)": "hybrid",
        ...
        st.markdown("---")
        st.markdown("## Step 3️⃣: Safety Settings")
        ...
        if paper_mode:
            ...
            st.error("⚠️ DANGER - This will use REAL MONEY. Only for experts!")
    ...
    try:
        ...
        with col1:
            st.markdown("### 📋 What I'm Trading Now")
        ...
        with st.expander("⚙️ Your Bot Settings"):
            ...
            st.write(f"**Mode:** {'Paper (Practice)' if bot.paper_mode else '⚠️ Real Money'}")
